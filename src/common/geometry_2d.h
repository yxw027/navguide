 ...
 * If the two segments intersect, stores the intersection point in %result.
 *
 * Returns: 0 if the two line segments intersect, and -1 if not.
 */
static inline int
...
{
    double d = (p3->y-p2->y)*(p1->x-p0->x) - (p3->x-p2->x)*(p1->y-p0->y);
    if (fabs (d) < GEOM_EPSILON) return -1;
    double ua_n = (p3->x-p2->x)*(p0->y-p2->y) - (p3->y-p2->y)*(p0->x-p2->x);
    double ua = ua_n / d;
    result->x = p0->x + ua * (p1->x - p0->x);
    result->y = p0->y + ua * (p1->y - p0->y);
...
/**
 * geom_polyline_compute_polygon_offset_2d:
 * @polyline:       the target polyline
 * @offset:         the offset distance
 * @ncircle_points: circular parts of the resulting offset curve will be
 *                  approximated by a series of line segments.  This parameter
 *                  specifies how many points to place in such a circle.  It
 *                  must be greater than 4, and a multiple of 2.  If this is
 *                  zero, then a reasonable default will be chosen.
 *
 * Computes a polygon offset of a polyline.  Any point within distance %offset
 * from the polyline is guaranteed to lie within the resulting polygon, and
 * most points that are greater than distance %offset from the polyline should 
 * lie outside the resulting polygon.
 *
 * Returns: a newly allocated polygon2d_t
 */
polygon2d_t *
geom_polyline_compute_polygon_offset_2d (const pointlist2d_t *polyline,
        double offset, int ncircle_points);

/**
 * geom_polygon_compute_polygon_offset_tapered_2d:
 * @polyline:        the target polyline
 * @start_offset:    the offset distance at the start of the polyline
 * @end_offset:      the offset distance at the end of the polyline
 *
 * Computes a polygon offset where the offset width at the start and end can be
 * different.
 */
polygon2d_t *
geom_polyline_compute_polygon_offset_tapered_2d (const pointlist2d_t *polyline,
        double start_offset, double end_offset, int ncircle_points);
