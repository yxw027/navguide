 ...
 */
polygon2d_t *
convexhull_simple_polygon_2d (const polygon2d_t *input)
{
    ...
    g_queue_pop_tail (q);
    polygon2d_t *result = polygon2d_new_from_glist (g_queue_peek_head_link (q));
    g_queue_free (q);
    return result;
}

int
angle_sort_func (const void *a_ptr, const void *b_ptr, void *user_data) 
{
 ...
 */
polygon2d_t * 
convexhull_graham_scan_2d (const polygon2d_t *input)
{
    if (input->npoints < 3) return NULL;
    ...
    GList *sorted = NULL;
    for (int i=0; i<input->npoints; i++) {
        sorted = g_list_append (sorted, (void*) &input->points[i]);
    }
    sorted = g_list_sort_with_data (sorted, angle_sort_func, (void*) pivot);

    GList *siter = sorted;

    GQueue *hull = g_queue_new ();
    g_queue_push_tail (hull, siter->data); siter = siter->next;
    g_queue_push_tail (hull, siter->data); siter = siter->next;
    int hull_size = 2;

    while (siter) {
        GList *tail_link = g_queue_peek_tail_link (hull);
        const point2d_t *candidate = (const point2d_t*) siter->data;
    ...
    polygon2d_t *result = 
        polygon2d_new_from_glist (g_queue_peek_head_link (hull));
    g_list_free (sorted);
    g_queue_free (hull);
    return result;
}
