...
int 
pixel_copy_8u_generic (uint8_t *src, int sstride, 
...
//    fprintf (stderr, 
//            "copy generic begin <%dx%d> <%d, %d> -> <%d, %d> (%d : %d) Bpp %d\n", 
//            width, height, src_x, src_y, dst_x, dst_y, sstride, dstride, 
//            bytes_per_pixel);

    int i;
    for (i=0; i<height; i++) {
        uint8_t *dst_row = dst + (dst_y + i) * dstride;
        uint8_t *src_row = src + (src_y + i) * sstride;

        memcpy (dst_row + dst_x * bytes_per_pixel, 
                src_row + src_x * bytes_per_pixel, 
                width * bytes_per_pixel);
    }
//    fprintf (stderr, "copy generic end\n");
    return 0;
}
