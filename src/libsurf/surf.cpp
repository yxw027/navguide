...
void Surf::getDescriptor()
{
  ...
  float scale, *desc, co, si;
  float dx1, dy1, mdx1, mdy1;
  float dx2, dy2, mdx2, mdy2;
  ...
  for (int i = -10; i < 10; i+=5)
  {
      for (int j = -10; j < 10; j+=5) 
      {
          ...
          if (DESC_SIZE == 64) {
              double dx = dx1+dx2;
              double dy = dy1+dy2;
              double mdx = mdx1+mdx2;
              double mdy = mdy1+mdy2;
              desc[count++] = dx;
              desc[count++] = dy;
              desc[count++] = mdx;
              desc[count++] = mdy;
              ...
              desc[count++] = mdy2;
              // store the current length^2 of the vector
              len += dx1*dx1 + dy1*dy1 + mdx1*mdx1 + mdy1*mdy1 + dx2*dx2 + dy2*dy2 + mdx2*mdx2 + mdy2*mdy2;
          } else {
              fprintf (stderr, "Invalid SURF descriptor size %d\n", DESC_SIZE);
              assert (0);
          }
