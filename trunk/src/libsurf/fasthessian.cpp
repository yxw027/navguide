...
void FastHessian::getIpoints(int desc_size)
{
    ...
    for(int i = 1; i < intervals-1; i += 3) {
      for(int r = border; r < i_height - border; r += 3*step) {
        for(int c = border; c < i_width - border; c += 3*step) {
          ...
          {
            interpolateExtremum(o, i_max, r_max, c_max, desc_size);
          }
        }
      }
    }  
...
void FastHessian::interpolateExtremum(int octv, int intvl, int r, int c, int desc_size)
{
  ...
  if( fabs( xi ) < 0.5  &&  fabs( xr ) < 0.5  &&  fabs( xc ) < 0.5 )
  {
    ...
    Ipoint ipt (desc_size);
    printf ("new point desc size: %d\n", ipt.size); fflush(stdout);
    assert (ipt.descriptor);
  ...
  printf ("fast hessian interpolation done.\n"); fflush(stdout);
}
