...
glutil_texture_new (int width, int height, int max_data_size)
{
    ...
    t = (GLUtilTexture*)malloc (sizeof (GLUtilTexture));
...
void
glutil_texture_draw_color4f (GLUtilTexture * t, double r, double g, double b,
        double a)
{
        ...
        if (data) {
            uint8_t *buffer_data = 
                (uint8_t*) glMapBufferARB (GL_PIXEL_UNPACK_BUFFER_ARB,
                        GL_WRITE_ONLY);
            /*if (!buffer_data) { // bad idea...
                fprintf (stderr,"error allocating texture data\n");
                return -1;
                }*/ 
...
int glutil_texture_get_max_data_size (GLUtilTexture *t) { return t->max_data_size; }
