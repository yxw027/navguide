...
int main(int argc, char **argv)
{
    ...
    SiftImage sift_img = CreateImage (height, width);
    
    int levels = 3;
    double sigma_init = 1.6;
    double peak_thresh = 0.04/levels;
    int doubleimsize = 0;
    double resize = 1.0;
    
    // setup the SIFT machinery
    SetSiftParameters( peak_thresh, doubleimsize, levels, sigma_init);
    memcpy (sift_img->pixels[0], image, width*height*sizeof(float));
    ...
    keys = GetKeypoints (sift_img, 1.0);
