...
#define GTKU_GL_DRAWING_AREA_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), GTKU_TYPE_GL_DRAWING_AREA, GtkuGLDrawingAreaPrivate))
typedef struct _GtkuGLDrawingAreaPrivate GtkuGLDrawingAreaPrivate;

struct _GtkuGLDrawingAreaPrivate {
    Display * dpy;
    XVisualInfo * visual;
    GLXContext context;
...
static void gtku_gl_drawing_area_realize (GtkWidget * widget);
static void gtku_gl_drawing_area_unrealize (GtkWidget * widget);
static void gtku_gl_drawing_area_size_allocate (GtkWidget * widget,
        GtkAllocation * allocation);

G_DEFINE_TYPE (GtkuGLDrawingArea, gtku_gl_drawing_area, GTK_TYPE_DRAWING_AREA);

static void
gtku_gl_drawing_area_class_init (GtkuGLDrawingAreaClass * klass)
{
    GtkWidgetClass * widget_class = GTK_WIDGET_CLASS (klass);
    GObjectClass * gobject_class = G_OBJECT_CLASS (klass);

    //widget_class->expose_event = gtku_gl_drawing_area_expose;
    widget_class->realize = gtku_gl_drawing_area_realize;
    widget_class->unrealize = gtku_gl_drawing_area_unrealize;
    widget_class->size_allocate = gtku_gl_drawing_area_size_allocate;

    g_type_class_add_private (gobject_class, sizeof (GtkuGLDrawingAreaPrivate));
}
...
GtkWidget *
gtku_gl_drawing_area_new (gboolean vblank_sync)
{
    GObject * object = g_object_new (GTKU_TYPE_GL_DRAWING_AREA, NULL);
    GtkuGLDrawingArea * self = GTKU_GL_DRAWING_AREA (object);
    self->vblank_sync = vblank_sync;
    return GTK_WIDGET (object);
}

void 
gtku_gl_drawing_area_set_vblank_sync (GtkuGLDrawingArea *self, 
...
{
    GtkuGLDrawingAreaPrivate * priv = (GtkuGLDrawingAreaPrivate *) arg;
    ...
    while (!priv->quit_thread) {
        ...
        size_t size = write (priv->pipe[1], "+", 1);
        assert (size >= 0);
        usleep (10);
        odd_even = !odd_even;
    }
...
static void
gtku_gl_drawing_area_unrealize (GtkWidget * widget)
{
    GtkuGLDrawingArea * self = GTKU_GL_DRAWING_AREA (widget);
    GtkuGLDrawingAreaPrivate * priv = GTKU_GL_DRAWING_AREA_GET_PRIVATE (self);
    ...
    /* chain up */
    GTK_WIDGET_CLASS (gtku_gl_drawing_area_parent_class)->unrealize (widget);
}
...
int
gtku_gl_drawing_area_set_context (GtkuGLDrawingArea * self)
{
    GtkWidget * widget = GTK_WIDGET (self);
    GtkuGLDrawingAreaPrivate * priv = GTKU_GL_DRAWING_AREA_GET_PRIVATE (self);
